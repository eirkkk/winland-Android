    ...
    D: 'static,
{
    fn bind(
        ...
        log::info!(
            "WinlandSeatBind: entering bind handler, name={:?}, ptr_ref={:p}",
            self.arc.name,
            Arc::as_ptr(&self.arc)
        );
        ...
        log::info!(
            "WinlandSeatBind: resource initialized, version={}",
            resource.version()
        );

        if resource.version() >= 2 {
            resource.name(self.arc.name.clone());
            log::info!("WinlandSeatBind: name event sent: {:?}", self.arc.name);
        } else {
            log::info!("WinlandSeatBind: version<2, skipping name event");
        }

        let mut inner = match self.arc.inner.lock() {
            Ok(guard) => guard,
            Err(poisoned) => {
                log::error!("WinlandSeatBind: inner lock POISONED, recovering");
                poisoned.into_inner()
            }
        };
        let caps = inner.compute_caps();
        log::info!(
            "WinlandSeatBind: compute_caps()={:?}  (pointer={}, keyboard={}, touch={})",
            caps,
        ...
        log::info!("WinlandSeatBind: bind handler complete");
    }
}
