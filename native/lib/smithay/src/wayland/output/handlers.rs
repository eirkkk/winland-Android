    ...
    D: 'static,
{
    fn bind(
        ...
        log::info!(
            "SmithayRuntime: WlOutput bind handler: make={:?} model={:?} phys=({},{}) modes={:?} cur_mode={:?} name={:?}",
            inner.physical.make, inner.physical.model,
            inner.physical.size.w, inner.physical.size.h,
            inner.modes, inner.current_mode, inner.name
        );
        ...
        drop(inner);
        log::info!("SmithayRuntime: WlOutput bind handler calling output_bound");
        state.output_bound(self.output.clone(), output);
        log::info!("SmithayRuntime: WlOutput bind handler complete");
    }
}
