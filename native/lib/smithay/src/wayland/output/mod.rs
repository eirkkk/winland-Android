...
impl Inner {
    fn send_geometry_to(&self, output: &WlOutput) {
        log::info!(
            "SmithayRuntime: send_geometry_to output={:?} make={:?} model={:?} phys=({},{}) loc=({},{})",
            output.id(),
            self.physical.make, self.physical.model,
            self.physical.size.w, self.physical.size.h,
...
impl Output {
    ...
    pub(crate) fn wl_change_current_state(
        ...
        for output in &inner.instances {
            ...
            let Some(data) = output.data::<OutputUserData>() else {
                continue;
            };
            ...
            if new_transform.is_some() || new_location.is_some() {
                log::info!(
                    "SmithayRuntime: wl_change_current_state re-sending geometry to output={:?}",
                    output.id()
                );
            ...
            if new_mode.is_some() {
                log::info!(
                    "SmithayRuntime: wl_change_current_state re-sending mode to output={:?} mode={:?}",
                    output.id(), new_mode
                );
            }
